...
class FileOperationsService {
  ...
  async removeFileWithChoice(filePath, description = 'File', isDryRun = false) {
    ...
    if (success) {
      try {
        ...
        this.addOperationLog('trash', filePath, null, stats.size);
      } catch (error) {
        // File already deleted, log with unknown size
        this.addOperationLog('trash', filePath, null, 0);
      }
    }
    return success;
  }
    ...
    if (scanResults.photoFiles) {
      for (const file of scanResults.photoFiles) {
        try {
          ...
          this.emitLog(`Photo file no longer exists: ${path.basename(file.path)}`);
        }
      }
    }
    ...
    if (scanResults.shortVideos) {
      for (const file of scanResults.shortVideos) {
        try {
          ...
          this.emitLog(`Short video no longer exists: ${path.basename(file.path)}`);
        }
      }
    }
  ...
  async performScan(config) {
    ...
    // Parse extensions
    const photoExtensions = config.photoExtensions.split(',').map(ext => ext.trim().toLowerCase());
    const videoExtensions = config.videoExtensions.split(',').map(ext => ext.trim().toLowerCase());
    ...
    this.emitLog('Scanning for empty folders...');
    const emptyFolders = await this.getEmptyDirectories(
      config.startFolder, 
      config.ignoreFolders,
      config.startFolder,
      (count, folderName) => {
        ...
        this.emitLog(`Found empty folder ${count}: ${folderName}`);
